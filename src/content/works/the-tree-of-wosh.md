---
title: "思いの木"
titleEn: "The Tree of Wosh"
year: 2014
yearDisplay: "2014−2024"
category: permanent
venue: 兵庫県
coverImage: /images/works/the-tree-of-wosh/cover.jpg
images:
  - /images/works/the-tree-of-wosh/01.jpg
  - /images/works/the-tree-of-wosh/02.jpg
  - /images/works/the-tree-of-wosh/03.jpg
  - /images/works/the-tree-of-wosh/05.jpg
  - /images/works/the-tree-of-wosh/06.jpg
  - /images/works/the-tree-of-wosh/07.jpg
oldPath: /2
---

人々の思いを受け入れる「空」の造形を、25個のキューブによって構成される「樹形」と後日利用者によって行われる行為とのコラボレーションによって表現。垂直水平の多い建築空間内に「傾いた形状」でアクセントを加え、ある場所から見ると一つの「面」が浮かび上がるよう計画。利用者が思いを綴る「葉」のカラーシートをキューブ上に配置することで徐々に色づき成長し、各面に「命」の文字を形作る。10年後に完成予定。
